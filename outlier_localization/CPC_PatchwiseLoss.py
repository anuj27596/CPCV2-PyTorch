from outlier_localization.PatchwiseInfoNCE_Loss import InfoNCE_Loss
...
class CPC(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.pred_directions):
            ...
            tl, pl, pc = self.pred_loss[i](self.encodings, self.contexts)
            total_loss += tl
            patchwise_loss += pl
            patchwise_count += pc

            patchwise_loss = patchwise_loss.transpose(1,2).flip(2)
            patchwise_count = patchwise_count.transpose(0,1).flip(1)

        total_loss /= self.pred_directions
        patchwise_loss /= patchwise_count

        return total_loss, patchwise_loss
