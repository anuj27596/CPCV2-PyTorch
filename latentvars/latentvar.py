...
if __name__ == '__main__':
	...
	df_path = 'bloodmnist-unshuffled-tsne-clusters.csv'
	...
	# df.to_csv('bloodmnist-unshuffled-tsne-clusters.csv', index=False)
	...
	centroids = np.array([X[cluster_ids == i].mean(axis=0) for i in range(4)])
	...
	sns.scatterplot(
		...
		hue="cluster", palette=sns.color_palette("bright", 4),
		data=df,
		alpha=0.75
	)
	...
	sns.scatterplot(
		x="tsne-2d-one", y="tsne-2d-two",
		hue="hd-cluster", palette=sns.color_palette("bright", 4),
		data=df,
		alpha=0.75
	)
